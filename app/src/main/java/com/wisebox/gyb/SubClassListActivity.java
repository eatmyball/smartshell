...
public class SubClassListActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		lvwCourse = (ListView) this.findViewById(R.id.lvwSubClassListData);
		lblMessage=(TextView)this.findViewById(R.id.lblSubClassListMessage);
		
		Intent intent = getIntent();
		strModuleID = intent.getStringExtra("moduleID");
		GetCourse();
		BindData2View();
	}
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		getMenuInflater().inflate(R.menu.sub_class_list, menu);
		return true;
	}
	...
	private void GetCourse() {
		...
		// 调用的方法名称
		String methodName = "GetCourseByModuleId";
		// EndPoint
		String endPoint = "http://3b53f1-0.sh.1251226507.clb.myqcloud.com/htmwstest/wstrain.asmx";
		// SOAP Action
		String soapAction = "http://tempuri.org/GetCourseByModuleId";
		...
		rpc.addProperty("moduleId", strModuleID);		
	...
	private void BindData2View() {
		...
		if (lstCourse != null) {
			for (int i = 0; i < lstCourse.size(); i++) {
				...
				map.put("lblMessage1", "二级课程ID: "+lstCourse.get(i).getModuleId());
				map.put("lblMessage2", "二级课程名: "+lstCourse.get(i).getModuleName());
				map.put("lblMessage3", "开课日期: "+lstCourse.get(i).getTrainDate()+" 参与人数:"+lstCourse.get(i).getCourseCount());
		...
		lvwCourse.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				bundle.putString("code", lstCourse.get(nSel).getModuleId());
				bundle.putString("name", lstCourse.get(nSel).getModuleName());
				bundle.putString("studentCount", lstCourse.get(nSel).getCourseCount());
				bundle.putString("trainDate", lstCourse.get(nSel).getTrainDate());
