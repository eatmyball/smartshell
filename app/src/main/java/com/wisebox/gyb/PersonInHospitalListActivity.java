...
public class PersonInHospitalListActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_person_in_hospital_list);
		
		lvwPerson = (ListView) this.findViewById(R.id.xmry_lvwPerson);
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		getMenuInflater().inflate(R.menu.person_in_hospital_list, menu);
		return true;
	}
	...
	private void GetPreson() {
		...
		// 调用的方法名称
		String methodName = "GetUserByHospital";
		// EndPoint
		String endPoint = "http://3b53f1-0.sh.1251226507.clb.myqcloud.com/htmwstest/wstrain.asmx";
		// SOAP Action
		String soapAction = "http://tempuri.org/GetUserByHospital";
	...
	private void BindListview() {
		...
		if (lstPerson != null) {
			for (int i = 0; i < lstPerson.size(); i++) {
				...
				map.put("lblMessage1", "员工ID: "+lstPerson.get(i).getCode());
				map.put("lblMessage2", "员工名称: "+lstPerson.get(i).getName());
				map.put("lblMessage3", "");
		...
		// 添加并且显示
		lvwPerson.setAdapter(listItemAdapter);

		// 添加点击
		lvwPerson.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				bundle.putString("code", lstPerson.get(nSel).getCode());
				bundle.putString("name", lstPerson.get(nSel).getName());
